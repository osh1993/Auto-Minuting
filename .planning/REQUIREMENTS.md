...
- [ ] **GEMINI-01**: 사용자가 설정 화면에서 Gemini API 키를 여러 개 추가/삭제할 수 있다
- [ ] **GEMINI-02**: 앱이 Gemini STT 또는 회의록 생성 호출 시 등록된 키를 라운드로빈 순서로 순환하여 사용한다
- [ ] **GEMINI-03**: 특정 API 키가 오류(권한 오류, 할당량 초과 등)를 반환하면 사용자에게 알림을 표시하고 자동으로 다음 키로 전환한다
- [ ] **GEMINI-04**: 등록된 모든 Gemini API 키가 암호화 저장된다

### MP3 파일 합치기

- [ ] **MERGE-04**: 사용자가 Share Intent로 여러 MP3 파일을 공유했을 때 앱이 자동으로 하나의 MP3 파일로 합쳐 처리한다 (재인코딩 없이)
- [ ] **MERGE-05**: Share Intent로 M4A와 MP3가 혼재된 파일 목록이 전달될 때 앱이 적절히 처리한다 (포맷별 분리 로직)
...
- [ ] **GROQ-01**: Groq Whisper STT 엔진 선택 시 입력 파일이 25MB를 초과하면 앱이 자동으로 파일을 청크로 분할한다
